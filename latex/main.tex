\documentclass[letterpaper,10pt]{article}
\usepackage[utf8]{inputenc}
%% To include CJK character, one needs to use xelatex to compile
%% and also to have a CJK font installed (current set to `Noto Sans CJK TC`)
%% Otherwise, comment out the following two lines.  
%\usepackage{xeCJK}
%\setCJKmainfont{Noto Sans CJK TC}

%% basic font settings
\usepackage[T1]{fontenc} 
\usepackage[default]{raleway}
\providecommand{\raleway}{} % in case we are not using xeLaTex
\newcommand*{\latinfont}{\raleway}
\hyphenpenalty=10000

%% essential packages
\usepackage[boxed]{ticket} %when ready to print, remove the "boxed" option
\usepackage[usenames]{xcolor}
\usepackage{graphicx} % provides \includegraphics
\usepackage{adjustbox} % provides \maxsizebox 

%% to use colored text for highlighting
\newcommand*{\highlight}{\textcolor}
%% OR, to use contour for highlighting, uncomment below
%\usepackage[outline]{contour} 
%\contourlength{5pt}
%\newcommand*{\highlight}{\contour}

%% other useful commands
\newcommand*{\cbox}[1]{\makebox[0pt]{\maxsizebox{3.8in}{!}{#1}}}
\newcommand*{\lbox}[1]{\makebox[0pt][l]{\maxsizebox{3.8in}{!}{#1}}}
\newcommand*{\rbox}[1]{\makebox[0pt][r]{\maxsizebox{3.8in}{!}{#1}}}
\newcommand*{\namefont}{\fontsize{24pt}{0pt}\selectfont}
\newcommand*{\affiliationFont}{\fontsize{18pt}{0pt}\selectfont}

%% page setting, currently set to match Avery Name Badges #74459
\hoffset=-0.25in %adjusting page margin %0.41
\voffset=-0.5in %adjusting page margin 
\unitlength=0.01in
\ticketSize{365.8}{236} % in unitlength
\ticketDistance{0}{0} %in unitlength
\ticketNumbers{2}{4} % how many badges per page

%% ticket default setting
\renewcommand*{\ticketdefault}{%
  \put(20,2){\lbox{\includegraphics[width=1.1in]{cmsb2}}}%
  %\put(380,14){\rbox{\includegraphics[width=0.7in]{lsstc-logo}}}%
  \put(180,220){\cbox{\latinfont\small 16th International Conference on }}%
  \put(180,205){\cbox{\latinfont\small Computational Methods in Systems Biology}}%  
  \put(240,35){\cbox{\latinfont\small 12th-14th September 2018}}% 
  \put(240,20){\cbox{\latinfont\small Faculty of Informatics, Brno}}%
}

%% participant style setting
\newcommand*{\participant}[4]{\ticket{%
  \put(180,155){\cbox{#4{\latinfont\bfseries\namefont #1}}}%
  \put(180,115){\cbox{\latinfont\affiliationFont #2}}%
  \put(180,85){\cbox{\latinfont\affiliationFont #3}}%
}}

%% empty ticket (with background)
\newcommand*{\emptyticket}{\ticket{}}

%% local people or other highlight commands
\newcommand*{\local}{\highlight{orange}}

%% backside printing (comment out for front side)
%\hoffset=1.1in
%\backside

\usepackage{csvsimple}


\begin{document}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% insert below the participant list
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\participant{Large-font Name}{Affiliation}{Azerbajdžan}{}
\participant{Uhura}{Starfleet}{USA}{}
\emptyticket{}
\participant{Jim}{Starfleet}{here}{\local}
\participant{Bones}{Starfleet}{there}{}
\emptyticket{}
\participant{Large-font Name}{Affiliation}{nowehre}{}
\participant{Uhura Loïc}{Starfleet}{}{}

%\csvreader[head to column names]{stav_reg.csv}{}% use head of csv as column names
%    {\participant{\name \surname}{\affiliace}}{}% specify your coloumns here

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% end of the participant list
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% empty tickets below
%\emptyticket{}
%\emptyticket{}
\end{document}
